...
| ทุกหน้า | 🔥 แท็บ **มาแรง** เหนือแถบเมนู แนะนำฟีเจอร์ที่คนเข้าดูมากที่สุด |
...
### 5. รันสคริปต์สำหรับแท็บ "มาแรง"

เปิด **SQL Editor → New query** วางไฟล์ [docs/supabase.sql](docs/supabase.sql)
ทั้งไฟล์แล้วกด Run (รันซ้ำได้ ไม่พัง)

สคริปต์นี้สร้างตาราง `feature_clicks` กับฟังก์ชัน `bump_feature_click`
ที่ใช้นับว่าฟีเจอร์ไหนคนเข้าดูมากที่สุด **ไม่รันก็ได้** — เว็บยังใช้ครบทุกอย่าง
แค่แท็บมาแรงจะไม่ขึ้นเท่านั้น

ตารางเก็บแค่ยอดรวมต่อฟีเจอร์ ไม่ได้เก็บว่าใครคลิก
และ anon key เขียนตารางตรงๆ ไม่ได้ ต้องผ่านฟังก์ชันที่บวกได้ทีละหนึ่งเท่านั้น
...
- ไอคอนทุกขนาดเป็นไฟล์ PNG นิ่งใน repo (`src/app/icon.png`, `public/icon-192.png`,
  `public/icon-512.png`) ไม่ได้สร้างตอน request แล้ว
  สร้างใหม่ได้ด้วย [docs/icon-preview/render-icons.ps1](docs/icon-preview/render-icons.ps1)
...
│   ├── PopularTab.tsx            แท็บ "มาแรง" + ตัวนับยอดเข้าดูแต่ละหน้า
...
│   ├── features.ts               รายการฟีเจอร์กลาง ใช้ร่วมกันทั้งเมนูและแท็บมาแรง
│   ├── popular.ts                อ่าน/เพิ่มยอดคลิกใน Supabase (ล้มแล้วเงียบ)
...
**อย่าเปลี่ยน `id` ใน [features.ts](src/lib/features.ts) พร่ำเพรื่อ** — id เป็นคีย์ของ
ตาราง `feature_clicks` ใน Supabase เปลี่ยนแล้วยอดคลิกเดิมจะกลายเป็นแถวกำพร้า
ยอดจะรีเซ็ตเป็นศูนย์ ถ้าจำเป็นต้องเปลี่ยนจริงๆ ต้อง `update` แถวเดิมตามไปด้วย

**เพิ่มหน้าฟีเจอร์ใหม่ ให้เพิ่มใน `FEATURES` ที่เดียว** — ทั้งแถบเมนูและแท็บมาแรง
อ่านจากรายการเดียวกัน ไม่ต้องไปแก้ `SiteHeader.tsx` อีก
