...
> คำทำนายทั้งหมดคำนวณในเว็บเองเพื่อความบันเทิง ไม่ได้ให้ AI แต่งให้
> ส่วนแชทน้องมูใช้ Gemini แต่ทำหน้าที่ไกด์เว็บเท่านั้น ไม่ได้ทำนายดวง
...
## ติดตั้งเป็นแอปในเครื่อง (PWA)

เว็บนี้ติดตั้งลงมือถือหรือคอมได้เหมือนแอปจริง

**Android / Chrome / Edge** — เข้าเว็บแล้วจะมีแถบ "ติดตั้งมูทูเดย์ไว้ในเครื่อง"
ขึ้นมาเอง กด **ติดตั้งเลย** ได้เลย หรือกดเมนู ⋮ แล้วเลือก *ติดตั้งแอป*

**iPhone / iPad** — Safari ไม่รองรับปุ่มติดตั้งอัตโนมัติ ต้องกดปุ่มแชร์ด้านล่าง
แล้วเลือก **เพิ่มไปยังหน้าจอโฮม** (เว็บจะขึ้นคำแนะนำนี้ให้เอง)

### ใช้ตอนออฟไลน์ได้แค่ไหน

| ทำได้ | ทำไม่ได้ |
|---|---|
| เปิดหน้าที่เคยเข้าแล้วตอนมีเน็ต | เข้าสู่ระบบ / สมัครสมาชิก |
| ดูสีมงคล ราศี เซียมซี ไพ่ ที่โหลดไว้แล้ว | แชทกับน้องมู |

หน้าที่ไม่เคยเปิดจะขึ้นหน้า `/offline` แทน

### หมายเหตุสำหรับคนแก้โค้ด

- ไอคอน 192/512 px สร้างเป็น PNG ตอน request ด้วย `ImageResponse` ที่มากับ Next.js
  ([src/lib/app-icon.tsx](src/lib/app-icon.tsx)) ไม่มีไฟล์รูปเก็บไว้ในโปรเจกต์
  ลายวาดด้วย `div` ล้วนไม่มีตัวอักษร เพราะ satori ต้องโหลดฟอนต์เองถ้ามีข้อความ
- **`/sw.js` และ `/manifest.webmanifest` ต้องอยู่นอก matcher ของ middleware**
  ถ้าเผลอให้ middleware ดัก จะถูก redirect ไปหน้า login แล้วติดตั้งแอปไม่ได้เลย
- แก้กลยุทธ์แคชเมื่อไหร่ ให้ขยับตัวแปร `VERSION` ใน [public/sw.js](public/sw.js)
  ไม่งั้นเครื่องที่เคยเข้าแล้วจะใช้ service worker ตัวเก่าต่อไป
